def acao_a_star(node):
    '''
    Entrada: representação de um nó -> [estado, pai, ação, heuristica, custo, função sucessora, profundidade]
    Saída: lista de nodes (que são as expansões deste)
    '''
    expansoes = []
    profundidade = node[6] + 1
...
def menor_f_n(fronteira: list):
    # node = [estado, pai, ação, heuristica, custo, f(n), profundidade]
    fs = [node[5] for node in fronteira]
...
def a_star(init_node: list, printar=True):
    '''
    Entrada: lista contendo um estado do vetor
    Saída: None
    '''    
    
    init_node = [init_node, None, None, 0, 0, 0, 0] # formato desejado -> [estado, pai, ação, heuristica, custo, f(n), profundidade]
    ...
    while continuar:
        ...
        # Primeiro caso
        if fronteira == None:
            fronteira = acao_a_star(init_node) # Cria a primeira fronteira
            ...
            # Caso onde há solução
                ...
                explored_node = menor_f_n(fronteira)
                ...
                # Caso onde nó ainda não foi explorado
                    ...
                    fronteira += acao_a_star(explored_node)
                    explored.append(explored_node)
