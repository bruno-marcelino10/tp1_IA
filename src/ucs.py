...
def menor_custo(fronteira: list):
    # node = [estado, pai, ação, custo, profundidade]
    custos = [node[3] for node in fronteira]
    for i in range(len(custos)):
        if custos[i] == min(custos):
            node_escolhido = fronteira[i]
            break
    
    return node_escolhido

def ucs(init_node: list, printar=True):
    '''
    ...
    while continuar:
        ...
        # Primeiro caso
        if fronteira == None:
            fronteira = acao_ucs(init_node) # Cria a primeira fronteira
            ...
            # Caso onde há solução
                ...
                explored_node = menor_custo(fronteira)
                ...
                # Caso onde nó ainda não foi explorado
                    ...
                    fronteira += acao_ucs(explored_node)
                    explored.append(explored_node)
